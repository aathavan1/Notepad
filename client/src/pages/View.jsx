...
function View() {
  ...
  useEffect(() => {
    axios.get(`https://notepad-server-aathavans-projects-17a4968c.vercel.app/view/${id}`)
      .then((response) => {
  ...
  const submit = async () => {
    try {
      reload()
      await axios.post('https://notepad-server-aathavans-projects-17a4968c.vercel.app/view/add', {
        uid: id,
        unote: content,
      });
      setContent('');
  ...
  const delfun = async (keys) => {
    try {
      setCount(count + 1); // Refresh notes by updating count
      await axios.delete(`https://notepad-server-aathavans-projects-17a4968c.vercel.app/view/delete/${keys}`);
    } catch (err) {
